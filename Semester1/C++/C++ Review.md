...
* 八进制 ：`cout << oct << i;` 
* 十进制 ：`cout << dec << i;` 
* 操纵输出域宽 ：`cout << setw(4) << i;`
* 定义特定的填充字符 ：`cout << setfill(*);`
* 定义浮点数的精度 ：`cout << setfill(*) << fixed << setprecision(2);` —— 输出两位小数，长度不足 `setw(10)`的部分用 ***** 填充，下面是以 `cout << setw(10) << a << endl;`的方式反映出前面所预设的格式
...
* 总结
  ...
  * 函数覆盖发生在继承层次中，该函数在父类中必须是 virtual，而子类的该函数必须与父类具有相同的参数列表
