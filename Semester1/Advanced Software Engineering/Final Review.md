...
从具体事物抽出,，概括出它们共同的方面、本质属性与关系等，而将个别的、非本质的方面、属性与关
系舍弃，这种思维过程，称为抽象。
...
* 如果一个节点 CNode 非叶子节点也非根节点，如果强行将它看成其任何一个祖先节点 ANode， 并当做ANode 使用，那么在哲学意义上，叫做多态性
...
* 面向对象方法成为主流开发方法。可以从下列 几个方面来分析其原因：
...
* 动态绑定：在一般与特殊关系中，子类是父类的一 个特例，所以父类对象可以出现的地方，也允许其子类对象出现。因此在运行过程中， 当一个对象发送消息请求服务时，要**根据接收对象的具体情况将请求的操作与实现的方法进行连接**，即动态绑定。
...
* **包含关系**：用例可以简单地包含其他用例具有的行为，并把它所包含的用例行为做为自身行为的一部分，这被称作包含关系。侧重表示被包含用例对执行者提供服务的间接性。
...
* **关联关系**：连接模型元素及连接示例，用一条实线来表示。对于两个相对独立的系统，当一个系统的实例与另一个系统的一些特定实例存在固定的对应关系时，这两个系统之间为关联关系。
...
* 回答（概念设计要回答的问题）：
  ...
  * What choices will be offered to users 能向用户􏰃供什么选择?
...
* 深度、宽度、扇入、扇出适中 􏰁 
  ...
  * 扇出：一个模块直接控制（调用）的模块数目。扇出过大说明模块过分复杂；过小也不好，不
    利于系统平衡分解，3 到 9 为宜
...
构件设计将软件体系结构的结构元素变换为对软件构件的过程性􏰀述。从基于类的模型、动态模型和静态模型等获得的信息作为构件设计的基础。
...
* **OCP 开放关闭原则** 
  ...
  * 软件实体应该可以扩展，但不可以修改
  * 开放：从哲学上其实是要我们在设计和开发软件时􏰃高抽象层次，不要总在具体对象层面上进行处理。
  * 关闭：设计好的类一定要对修改关闭
